...
//------------------------Parameter----------------------
`define TV_IN_y "../tv/cdatafile/c.omp_reconstruction.autotvin_y.dat"
`define TV_IN_A "../tv/cdatafile/c.omp_reconstruction.autotvin_A.dat"
`define TV_IN_x_out "../tv/cdatafile/c.omp_reconstruction.autotvin_x_out.dat"
...
parameter x_out_DEPTH = 1;
reg [31 : 0] x_out_OPERATE_DEPTH = 0;
parameter x_out_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter omp_reconstruction_continue_addr = 0;
parameter omp_reconstruction_auto_start_addr = 0;
parameter y_data_in_addr = 16;
parameter A_data_in_addr = 28;
parameter x_out_data_in_addr = 40;
...
reg [x_out_c_bitwidth - 1 : 0] mem_x_out [x_out_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_x_out [ (x_out_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * x_out_DEPTH -1 : 0] = '{default : 'hz};
reg x_out_write_data_finish;
...
//write x_out reg
reg [31 : 0] write_x_out_count = 0;
reg write_x_out_run_flag = 0;
reg write_one_x_out_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & y_write_data_finish & A_write_data_finish & x_out_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        x_out_write_data_finish <= 0;
        write_x_out_run_flag <= 0; 
        write_x_out_count = 0;
        count_operate_depth_by_bitwidth_and_depth (x_out_c_bitwidth, x_out_DEPTH, x_out_OPERATE_DEPTH);
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            x_out_write_data_finish <= 0;
        ...
        if (AESL_ready_reg === 1) begin
            write_x_out_run_flag <= 1; 
            write_x_out_count = 0;
        end
        if (write_one_x_out_data_done === 1) begin
            write_x_out_count = write_x_out_count + 1;
            if (write_x_out_count == x_out_OPERATE_DEPTH) begin
                write_x_out_run_flag <= 0; 
                x_out_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_x_out
    integer write_x_out_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    integer i;
    integer j;
    reg [31 : 0] x_out_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = x_out_c_bitwidth;
    process_num = 3;
    count_c_data_four_byte_num_by_bitwidth (c_bitwidth , four_byte_num);
    ceil_align_to_pow_of_two_four_byte_num = ceil_align_to_pow_of_two(four_byte_num);
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_x_out_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write x_out data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (x_out_c_bitwidth < 32) begin
                        x_out_data_tmp_reg = mem_x_out[write_x_out_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < x_out_c_bitwidth) begin
                                x_out_data_tmp_reg[j] = mem_x_out[write_x_out_count][i*32 + j];
                            end
                            else begin
                                x_out_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_x_out[write_x_out_count * four_byte_num  + i]!==x_out_data_tmp_reg) begin
                    write (x_out_data_in_addr + write_x_out_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, x_out_data_tmp_reg, write_x_out_resp);
                    image_mem_x_out[write_x_out_count * four_byte_num + i]=x_out_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_x_out_data_done <= 1;
                @(posedge clk);
                write_one_x_out_data_done <= 0;
            end   
            process_3_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_x_out_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [x_out_c_bitwidth - 1 : 0] token_tmp; 
  reg [DATA_WIDTH - 1 : 0] tmp_cache_mem; 
  reg [ 8*5 : 1] str;
    reg [63:0] trans_depth;
  integer transaction_idx; 
  ...
  transaction_idx = 0; 
  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
  count_seperate_factor_by_bitwidth (x_out_c_bitwidth , factor);
  fp = $fopen(`TV_IN_x_out ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_x_out); 
      $finish; 
  end 
  read_token(fp, token); 
  if (token != "[[[runtime]]]") begin             // Illegal format 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < x_out_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  tmp_cache_mem [31 : 24] = token_tmp;
                  mem_x_out [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
